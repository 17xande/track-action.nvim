...
function M.resolve_rhs(rhs, desc)
  if not rhs or rhs == "" then
    return nil
  end

  config.debug("Mappings: resolving RHS: %s (desc: %s)", rhs, desc or "none")
...
function M.could_be_mapping(partial, mode)
  ...
  for lhs, _ in pairs(mapping_cache[mode]) do
...
function M.init()
  ...
  vim.api.nvim_create_autocmd({ "VimEnter", "BufEnter" }, {
    group = vim.api.nvim_create_augroup("MentorMappings", { clear = true }),
    callback = function()
